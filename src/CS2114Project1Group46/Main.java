...
public class Main {
    ...
    /**
     * Runs an interactive console workflow allowing the user to enter
     * salary, fixed expenses, and pending transactions dynamically.
     */
    public void runInteractiveConsole() {
        ...
        for (int i = 1; i <= numTransactions; i++) {
            ...
            pendingTransactions.add(new Transaction(amount, category, LocalDate.now(), note));
        }
        ...
        for (Transaction t : pendingTransactions) {
            double currentFlexible = calculator.computeFlexibleRemaining(profile, queryLoggedTransactions());
            Validator.ValidationResult result = validator.validateTransaction(t, currentFlexible, queryLoggedTransactions());

            if (result.isValid()) {
                history.addTransaction(t);
                System.out.println("[ACCEPTED] " + t.getCategory() + ": $" + t.getAmount() + " (" + t.getNote() + ")");
            } else {
                System.out.println("[REJECTED] " + t.getCategory() + ": $" + t.getAmount() + " -> Reason: " + result.getMessage());
            }
        }

        // 5. Compute Final Budget Metrics
        ArrayList<Transaction> logged = queryLoggedTransactions();
        double remainingSurplus = calculator.computeSurplusDeficit(profile, logged);
        double flexibleRemaining = calculator.computeFlexibleRemaining(profile, logged);
        ...
        System.out.println("=========================================");
        System.out.println("Total Accepted Transactions Logged: " + logged.size());
        System.out.println("Current Surplus/Deficit: $" + profile.getSurplusDeficit());
        System.out.println("Remaining Flexible Budget: $" + flexibleRemaining);

        // 6. Output Spending Percentages relative to total Salary
        Map<String, Double> percentages = calculator.computeCategoryPercentage(profile, logged);
    ...
    /**
     * Safely reads a double value from the console to prevent InputMismatchException.
     */
    ...
    /**
     * Safely reads an integer value from the console to prevent InputMismatchException.
     */
